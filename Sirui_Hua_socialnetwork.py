...
def create_data_structure(string_input):
    ...
    if type(string_input) != str:
        ...
        connection_start = "connected to "
        game_start = "to play "
        for i in range(0,len(split)-1,2):
            #find the string that only contains connection and split
            connection=(split[i][(split[i].find(connection_start)+13):].split(", "))
            #find the string that only contains games and split
            likes=(split[i+1][(split[i+1].find(game_start)+8):].split(", "))
            #put the connection and games into the key of the user
...
#define a helper procesure that returns network, begin user, end user, 
#and the list cheked in the procedure
def friend_path(network, user_A, user_B, checked):
    ...
    for user in get_connections(network, user_A):
        #check if all the users are in the cheked list
...
#game_in_common(network,user_A, user_B):
# find the games of two users in common, return the number of games in common 
# and names of the games.
def game_in_common(network,user_A, user_B):
    if user_A not in network or user_B not in network:
        return None
    else:
        result = []
        game_A = get_games_liked(network,user_A)
        game_B = get_games_liked(network,user_B)
        for game in game_A:
            if game in game_B:
                result.append(game)
